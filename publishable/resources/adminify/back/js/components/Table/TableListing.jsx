...
import TableBase from '@/back/js/components/Table/TableBase';
import useTableComponentRegistrar from '@/back/js/hooks/useTableComponentRegistrar';
import useTableSearch from '@/back/js/hooks/useTableSearch';
    ...
    return <>
        <div ref={ref} className='card'>
            <div className='card-header'>
                <div className='row'>
                    {components['areas']['top-left'].length > 0 && 
                        <div className='col'>
                            {components['areas']['top-left'].map(({Component, Vars, Global}) => (
                    ...
                    {components['areas']['top-right'].length > 0 && 
                        <div className='col'>
                            {components['areas']['top-right'].map(({Component, Vars, Global}) => (
                                <Component global={Global} datas={Vars} />
                            ))}
                        </div>
                    }
            ...
            <div className='card-body'>
                <TableBase hover striped bordered datas={props.datas} components={components.listing} actions={components.actions} />
            </div>
                ...
                <div className='card-footer'>
                    <div className='row'>
                        {components['areas']['bottom-left'].length > 0 && 
                            <div className='col'>
                                {components['areas']['bottom-left'].map(({Component, Vars, Global}) => (
                                    <Component global={Global} datas={Vars} />
                                ))}
                            </div>
                        }
                        {components['areas']['bottom-right'].length > 0 && 
                            <div className='col'>
                                {components['areas']['bottom-right'].map(({Component, Vars, Global}) => (
                                    <Component global={Global} datas={Vars} />
                                ))}
                            </div>
                        }
