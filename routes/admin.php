...
Route::prefix('admin')->middleware(['auth', 'role:administrator|editor|subscriber'])->as('admin.')->group(function () use ($c) {
    ...
    if(isset($c['menu']) && $c['menu']) {
        ...
        Route::post('menus/set-items-to-menu/{type}', 'App\Adminify\Http\Controllers\Back\MenuController@setItemsToMenu')->name('menus.setItemsToMenu');
    ...
    Route::put('{type}/trash/{id}', 'App\Adminify\Http\Controllers\Back\TrashController@update')->name('trash.update');
    Route::post('/{type}/duplicate/{id}', 'App\Adminify\Http\Controllers\Back\CopyController@index')->name('copy.entity');
    Route::get('/manipulate/{id}', 'App\Adminify\Http\Controllers\Back\MediaManipulationController@index')->name('manipulate.index');
    Route::post('/manipulate/{id}', 'App\Adminify\Http\Controllers\Back\MediaManipulationController@post')->name('manipulate.post');
    Route::delete('/manipulate/{id}', 'App\Adminify\Http\Controllers\Back\MediaManipulationController@destroy')->name('manipulate.destroy');
