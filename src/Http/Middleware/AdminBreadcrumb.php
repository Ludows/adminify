...
class AdminBreadcrumb
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        \Breadcrumbs::for($routeName, function ($trail) {
            ...
            $trail->push( _i('home.dashboard') , route('home.dashboard'));
            if(Str::contains($requestedNameRoute, 'index')) {
                $trail->push( _i($routeSpl['0'].'.index') , route($routeSpl['0'].'.index'));
            }
            if(Str::contains($requestedNameRoute, 'create') ) {
                $trail->push( _i($requestedNameRoute) , route($requestedNameRoute));
            }
            ...
            if(Str::contains($requestedNameRoute, 'edit')) {
                ...
                $trail->push( _i($requestedNameRoute) , route($requestedNameRoute, [$singular => $model->id]));
            }
