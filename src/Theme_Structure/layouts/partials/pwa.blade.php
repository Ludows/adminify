@if ($enabled_features['pwa'])
    <!-- Web Application Manifest -->
    {{-- {{ dd($pwa) }} --}}
    <link rel="manifest" href="{{ route('manifest') }}">
    <!-- Chrome for Android theme color -->
    <meta name="theme-color" content="{{ $pwa['_settings_pwa_theme_color'] }}">

    <!-- Add to homescreen for Chrome on Android -->
    <meta name="mobile-web-app-capable" content="{{ $pwa['_settings_pwa_display'] == 'standalone' ? 'yes' : 'no' }}">
    <meta name="application-name" content="{{ $pwa['_settings_pwa_shortname'] ?? '' }}">
    <link rel="icon" sizes="{{ data_get(end( $pwa['_settings_pwa_icons']), 'sizes') }}" href="{{ data_get(end($pwa['_settings_pwa_icons']), 'src') }}">

    <!-- Add to homescreen for Safari on iOS -->
    <meta name="apple-mobile-web-app-capable" content="{{ $pwa['_settings_pwa_display'] == 'standalone' ? 'yes' : 'no' }}">
    <meta name="apple-mobile-web-app-status-bar-style" content="{{  $pwa['_settings_pwa_status_bar'] ?? 'black' }}">
    <meta name="apple-mobile-web-app-title" content="{{ $pwa['_settings_pwa_shortname'] ?? '' }}">
    <link rel="apple-touch-icon" href="{{ data_get(end($pwa['_settings_pwa_icons']), 'src') }}">
