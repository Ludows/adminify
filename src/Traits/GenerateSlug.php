...
trait GenerateSlug
  {
   ...
   public function shouldGenerateSlug() {
       ...
       if(!$schemaBuilder->hasColumn($table, $this->appendSlugTo) && $useSlug) {
            throw new Exception($this->appendSlugTo.' required column for slug generation does not exist.');
       }
       if(!$schemaBuilder->hasColumn($table, $this->source) && $useSlug) {
        throw new Exception($this->source.' source column for slug generation does not exist.');
       }
